    ...
    <div class="p-4">
        <form action="{{ route('users.update', $user->id) }}" method="post" class="w-full mx-auto grid grid-flow-row gap-6">
            ...
            <div class="flex flex-col justify-between w-full">
                ...
                <input type="text" name="email" id="" class="rounded-md border border-gray-300" value="{{ $user->email }}">
            </div>
            <div class="flex flex-row items-start space-x-16">
                <div class="flex flex-col space-y-2">
                    ...
                    @foreach ($roles as $role)
                        <div class="flex flex-row items-center space-x-4">
                            <input type="checkbox" name="role[]" value="{{ $role->name }}" {{ $user->hasRole($role->name) == true ? 'checked' : '' }} id="" class="border border-gray-300">
                ...
                <div class="flex flex-col items-start space-y-2">
                    ...
                    @foreach ($permissions as $permission)
                        <div class="flex flex-row items-center space-x-4">
                            <input type="checkbox" name="permission[]" value="{{ $permission->name }}" {{ $user->hasPermissionTo($permission->name) == true ? 'checked' : '' }} id="" class="border border-gray-300">
